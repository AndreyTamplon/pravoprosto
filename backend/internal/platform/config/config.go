...
func Load() Config {
	return Config{
		...
		TBankNotificationPath:     getEnv("PRAVO_TBANK_NOTIFICATION_PATH", "/api/payment/webhook"),
		TBankSuccessURL:           getEnv("PRAVO_TBANK_SUCCESS_URL", ""),
		TBankFailURL:              getEnv("PRAVO_TBANK_FAIL_URL", ""),
		TBankPendingTTL:           time.Duration(getEnvInt("PRAVO_TBANK_PENDING_TTL_MINUTES", 60)) * time.Minute,
		TBankReceiptEnabled:       getEnvBool("PRAVO_TBANK_RECEIPT_ENABLED", false),
		TBankReceiptTaxation:      getEnv("PRAVO_TBANK_RECEIPT_TAXATION", ""),
		TBankReceiptTax:           getEnv("PRAVO_TBANK_RECEIPT_TAX", "none"),
		TBankReceiptPaymentMethod: getEnv("PRAVO_TBANK_RECEIPT_PAYMENT_METHOD", "full_payment"),
		TBankReceiptPaymentObject: getEnv("PRAVO_TBANK_RECEIPT_PAYMENT_OBJECT", "service"),
