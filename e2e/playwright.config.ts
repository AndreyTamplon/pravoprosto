...
export default defineConfig({
  ...
  webServer: [
    ...
    {
      command: `bash -c "PGPASSWORD=postgres psql -h localhost -p 5432 -U postgres -d postgres -q -c \\"SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = 'pravoprost_e2e'\\" 2>/dev/null; PGPASSWORD=postgres psql -h localhost -p 5432 -U postgres -d postgres -q -c 'DROP DATABASE IF EXISTS pravoprost_e2e' 2>/dev/null; PGPASSWORD=postgres psql -h localhost -p 5432 -U postgres -d postgres -q -c 'CREATE DATABASE pravoprost_e2e' 2>/dev/null; cd ../backend && ./server"`,
